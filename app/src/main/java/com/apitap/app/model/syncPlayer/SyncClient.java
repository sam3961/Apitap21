...
import com.apitap.app.views.fragments.checkinTv.FragmentStreamDetail;
...
public class SyncClient implements Runnable {
    ...
    private FragmentStreamDetail playerActivity;

    private Thread thr;

    private InetAddress addr;

    public SyncClient(FragmentStreamDetail playerActivity, InetAddress addr) {
        this.addr = addr;
        ...
        while (running.get()) {
            try {
                ...
                if (command == SyncCommand.SYNC.ordinal()) {
                    playerActivity.getActivity().runOnUiThread(() -> playerActivity.seekTo(val));
                } else if (command == SyncCommand.PLAY.ordinal()) {
                    playerActivity.getActivity().runOnUiThread(() -> {
