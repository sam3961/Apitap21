...
import com.apitap.app.views.fragments.specials.data.OrderByIdResponse
...
interface ApiService {
    ...
    @POST("aioproducts/itemInventoryByProductId/{productId}")
    suspend fun getInventoryByProductId(
        @Path("productId") productId: Int?
    ):  List<InventoryResponse>


    @GET("aioproducts/promotionsByCompanyId/{locationId}/{companyId}")
    fun getPromotionsByCompanyId(
        @Path("locationId") locationId: Int?,
        @Path("companyId") companyId: Int?,
    ): Call<List<PromotionListingResponse>>

    @GET("aioproducts/activeProductsByCompanyId/{companyId}")
    fun activeProductsByCompanyId(
        @Path("companyId") companyId: Int?,
    ): Call<List<AllProductsListResponse>>

    @GET("aioproducts/productById/{id}")
    fun getProductById(
        @Path("id") id: Int?,
    ): Call<ProductByIdResponse>


    @GET("aioorders/deleteOrderDetail/{tableOrder}/{detailId}/{productRegularPrice}/{elapsedTime}")
    ...
    @GET("aioorders/orderById/{id}/{locationId}")
    fun getOrderById(
        @Path("id") id: Int?,
        @Path("locationId") locationId: Int?
    ...
    @GET("aioproducts/promotionById/{promotionById}")
    fun promotionById(
        @Path("promotionById") promotionById: Int?,
    ): Call<List<PromotionListingResponse>>

    @GET("aioproducts/getActiveItemsByIds/{ids}")
